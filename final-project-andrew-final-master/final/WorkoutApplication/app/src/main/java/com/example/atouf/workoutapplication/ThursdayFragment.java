...
public class ThursdayFragment extends Fragment {
    ...
    private void randomize() {
        ...
        try {
            c = myDBHelper.rawQuery("select 1 as SortOrder,* from ( select * from workouts where musclegroup=? order by random() limit 4 ) union select 2 as SortOrder, * from ( select * from workouts where musclegroup=? order by random() limit 4 ) union select 3 as SortOrder, * from ( select * from workouts where musclegroup=? order by random() limit 2 ) order by SortOrder", new String[]{"Biceps", "Triceps", "Abs"});
            if (c.moveToFirst()) {
                do {
                    getSetsAndReps();
