...
public class WednesdayFragment extends Fragment {
    ...
    private void randomize() {
        ...
        try {
            c = myDBHelper.rawQuery("select 1 as SortOrder,* from ( select * from workouts where musclegroup=? order by random() limit 5 ) union select 2 as SortOrder, * from ( select * from workouts where musclegroup=? order by random() limit 2 ) order by SortOrder", new String[]{"Shoulders", "Abs"});
            if (c.moveToFirst()) {
                do {
                    getSetsAndReps();
